...
interface HeaderProps {
  ...
  onToggle: () => void;
  isSearchVisible?: boolean; // New optional prop to control search bar visibility
}

const Header: React.FC<HeaderProps> = ({ onClick, onToggle, isSearchVisible = true }) => {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      // Only focus if the search bar is visible
      if (isSearchVisible && (event.metaKey || event.ctrlKey) && event.key === "k") {
  ...
  }, [isSearchVisible]); // Add isSearchVisible to dependency array
    ...
    <header className="sticky top-0 flex w-full bg-white border-gray-200 z-99999 dark:border-gray-800 dark:bg-gray-900 lg:border-b">
      <div className="flex flex-col items-center justify-between flex-grow lg:flex-row lg:px-6">
        <div className="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4">
          ...
          {/* End of conditional rendering for search bar */}
        </div>
        <div
          className={`${
